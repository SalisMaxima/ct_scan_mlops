...
>
> Answer: Yes, we implemented monitoring using Prometheus metrics. Our FastAPI application uses prometheus-fastapi-instrumentator to automatically collect HTTP-level metrics (request counts, latencies, response sizes) exposed at the `/metrics` endpoint. Additionally, we implemented a custom Prometheus Counter to count how many our model makes wrrong prediction using feedback from users in the frontend. While we instrumented the API with metrics, we did not fully set up GCP Cloud Monitoring dashboards or alerting rules instaed we created SOLs for latency and availability ![latency and availability SOL](figures\gcloud-SLO.png), and we created an alert for latency if it is more than one second ![Latency Alert](figures\Alert_request_latency.png). The metrics endpoint is ready for scraping by Prometheus, which could then feed into Grafana for visualization or alerting.
